...
--------
	...
	shinchoku/js/jquery.iframe-transport.js ... jQuery File Upload v9.5.7
...
5. shinchokuディレクトリを公開場所にコピーする。`cp -p`を使うなどして所有権は変えずにコピーする。

メンテナンス
--------
* 定期実行用のクエリと当日集計用のクエリはほぼ同じ形になっています。クエリを修正する必要がある場合、下記の2つを必ず確認するようにしてください。

		production_program/shinchoku.sql
		shinchoku/shinchoku_ajax.php
